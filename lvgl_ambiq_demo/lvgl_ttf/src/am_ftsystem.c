...
FT_New_Memory(void)
{
    ...
    memory = (FT_Memory)am_mem_ssram_malloc(sizeof(*memory));
    if(memory) {
        memory->user    = NULL;
         ...
         long       size)
{
    ...
    return am_mem_ssram_malloc((size_t)size);
}
           ...
           void   *   block)
{
    ...
    return am_mem_heap_realloc(&ssram_heap, block, (size_t)new_size);
}
